...
public:
	...
	void setAnnualSalary(string sal) { annualSalary = stod(sal.c_str()); }
...
public:
	Salaried() {}
	Salaried(string first, string last, int ID, char gen, int dep, double sal, Benefit bfit, int manLevel) { firstName = first; lastName = last; empID = ID; gender = gen; dependents = dep; annualSalary = sal; benefit = bfit; managementLevel = manLevel; }
	...
	void setManagementLevel(string manLevel) { managementLevel = atoi(manLevel.c_str()); }
	void displayEmployee()
	{
		cout << "Name:              " << firstName << " " << lastName << endl;
		cout << "Employee ID:       " << empID << endl;
...
public:
	...
	Hourly(string first, string last, int ID, char gen, int dep, Benefit bfit, double wage, double hours, string category) { firstName = first; lastName = last; empID = ID; gender = gen; dependents = dep; benefit = bfit; this->wage = wage; this->hours = hours; this->category = category; }
	double calcPay() { double weekly; weekly = wage * hours; return weekly; }
	double getAnnualSalary() { return annualSalary; }
	void setAnnualSalary(double sal) { annualSalary = sal; sal = calcPay() * 50; }
